...
#define NAME "D:\\Documents\\programming\\bram_playground\\frame_dropper\\24-out.mp4"
#define OUT_NAME "D:\\Documents\\programming\\bram_playground\\motion_interpolator_cuda\\x64\\Debug\\24-out-extended.mp4" 
#define NUM_INTERP 8
#define ONE_NUM_INTERP 0.125
int main(int argc, char** argv)
{
	Mat frame, prevframe;
	Mat prevgray, gray;
	Mat fflow, bflow;
	string videoName = NAME;  // the video filename should be given as the arguement while executing the program
	...
	int iWidth = int(dWidth) - 1;
	int iHeight = int(dHeight) - 1;
	int fps = (int)capture.get(CV_CAP_PROP_FPS);


	Size frameSize(static_cast<int>(dWidth), static_cast<int>(dHeight));
	VideoWriter oVideoWriter(OUT_NAME, CV_FOURCC('X', 'V', 'I', 'D'), fps, frameSize, true); //initialize the VideoWriter object
	...
	for (;;)
	{
		...
		if (prevframe.data)
		{
			...
			fbc.process(prevgray, gray, fflow);
			fbc.process(gray, prevgray, bflow);
			for (int t = 0; t < NUM_INTERP; t++)     // interpolating 20 frames from two given frames at different time locations
			{
				...
				for (int y = 0; y < frame.rows; y++) //column scan
				{
					for (int x = 0; x < frame.cols; x++) //row scan
					{
						...
						fy = CLAMP(y + fxy.y*ONE_NUM_INTERP*t, 0, iHeight);
						fx = CLAMP(x + fxy.x*ONE_NUM_INTERP*t, 0, iWidth);

						flowf.at<Vec3b>(fy, fx) = prevframe.at<Vec3b>(y, x);

						const Point2f bxy = bflow.at<Point2f>(y, x);
						by = CLAMP(y + bxy.y*(1 - ONE_NUM_INTERP * t), 0, iHeight);
						bx = CLAMP(x + bxy.x*(1 - ONE_NUM_INTERP * t), 0, iWidth);
						flowb.at<Vec3b>(by, bx) = frame.at<Vec3b>(y, x);
					}
				}
				final = flowf * (1 - ONE_NUM_INTERP * t) + flowb * ONE_NUM_INTERP*t;  //combination of frwd and bckward martrix
				//cv::medianBlur(final, final, 3);
				oVideoWriter.write(final);
				/*cv::imshow("result", final);
				if (waitKey(1) > 0)break;*/
			}
			cout << "frames left :" << --numOfFrames << endl;
		}
